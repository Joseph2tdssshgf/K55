...
const isApiKeySet = (): boolean => {
  // In a no-build environment, process.env is attached to the window object by the host.
  const env = (window as any).process?.env;
  return !!sessionStorage.getItem('gemini-api-key') || !!env?.API_KEY;
}
...
const getAIInstance = (): GoogleGenAI => {
    ...
    const env = (window as any).process?.env;
    const apiKey = sessionStorage.getItem('gemini-api-key') || env?.API_KEY;
...
const App: React.FC = () => {
    ...
    const [isApiKeyRequired, setIsApiKeyRequired] = useState<boolean>(true); // Default to true
    const chatInstances = useRef<Map<string, Chat>>(new Map());

    useEffect(() => {
        // Delay check to allow environment variables to be injected by the host.
